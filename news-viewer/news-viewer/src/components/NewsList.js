...
const NewsList = ({ category }) => {
    ...
    }, [category]);             // 마지막에 [] 를 넣어야 최초 렌더링 시에만 비동기 요청 자동 호출 (category 상태변수 변경시 렌더링됨)


    // 대기 중일 때
    if(loading) {               // loading 변수로 참, 거짓 판단이 가능하니 바로 조건식에 대입
        return <div className={styles.newslistblock}>대기중...</div>
    }

    // articles 값이 저장되지 않았을 떄
    if(!articles) {
        return null;
    }

    // articles 값이 유효하게 처리되었을 때
    return (
